#include <string.h>
#include "file2img/blockFormatHandlers.h"
#include "file2img/colorUtils.h"

static f2i_rgba8888_t mix(f2i_rgba8888_t x, f2i_rgba8888_t y, int w0, int w1) {
	return (f2i_rgba8888_t){
...
static void populateDXTColorTable(uint16_t c0, uint16_t c1, f2i_rgba8888_t* colorTable) {
	colorTable[0] = f2i_convertRGB565(c0);
	colorTable[1] = f2i_convertRGB565(c1);
	if (c0 > c1) {
		...
		colorTable[3] = (f2i_rgba8888_t){0,0,0,0};
	}
}
...
void f2i_blockFormatHandlerDXT1(f2i_rgba8888_t* dst, uint8_t const* src, int dstWidth, int numBytes) {
	...
	for (int blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
		...
		uint16_t const c0 = f2i_readU16LE(&src);
		uint16_t const c1 = f2i_readU16LE(&src);
		uint32_t indexTable = f2i_readU32LE(&src);

		f2i_rgba8888_t colorTable[4];
...
void f2i_blockFormatHandlerDXT5(f2i_rgba8888_t* dst, uint8_t const* src, int dstWidth, int numBytes) {
	...
	for (int blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
		...
		uint64_t alphaIndexTable = f2i_readU48LE(&src);
		...
		for (int i = 0; i < 16; ++i) {
			...
			dst[y*dstWidth+x] = (f2i_rgba8888_t){
	...
	for (int blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
		...
		for (int iy = 0; iy < 8; ++iy) {
			...
			for (int ix = 0; ix < 8; ++ix) {
				uint8_t const val = f2i_expand2(((loBit>>(7-ix))&1) | (((hiBit>>(7-ix))&1)<<1));
				int const x = blockX + ix;
				int const y = blockY + iy;
				dst[y*dstWidth+x] = (f2i_rgba8888_t){val,val,val,0xff};
			}
		}
	}
}
...
void f2i_blockFormatHandlerNES(f2i_rgba8888_t* dst, uint8_t const* src, int dstWidth, int numBytes) {
	...
	for (int blockIndex = 0; blockIndex < blockCount; ++blockIndex) {
		...
		for (int iy = 0; iy < 8; ++iy) {
			for (int ix = 0; ix < 8; ++ix) {
				uint8_t const val = f2i_expand2(((loBits[iy]>>(7-ix))&1) | (((hiBits[iy]>>(7-ix))&1)<<1));
				int const x = blockX + ix;
				int const y = blockY + iy;
				dst[y*dstWidth+x] = (f2i_rgba8888_t){val,val,val,0xff};
			}
